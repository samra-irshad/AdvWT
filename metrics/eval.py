...
from metrics.id_loss import calculate_frs_given_images
...
def calculate_metrics(nets, args, step, mode):
    ...
    for trg_idx, trg_domain in enumerate(domains):
        ...
        for src_idx, src_domain in enumerate(src_domains):
            ...
            task = '%s2%s-%s' % (src_domain, trg_domain, mode)
            ...
            for i, x_src in enumerate(tqdm(loader_src, total=len(loader_src))):
                ...
                frs_value = calculate_frs_given_images(x_src, group_of_images)
                frs_values.append(frs_value)

            # calculate LPIPS for each task (e.g. cat2dog, dog2cat)
            lpips_mean = np.array(lpips_values).mean()
            lpips_dict['LPIPS_%s/%s' % (mode, task)] = lpips_mean
            # calculate FRS for each task (e.g. male2female)
            frs_mean = np.array(frs_values).mean()
            frs_dict['FRS_%s/%s' % (mode, task)] = frs_mean
    ...
    # report FRS values
    filename = os.path.join(args.eval_dir, 'FRS_%.5i_%s.json' % (step, mode))
    utils.save_json(frs_dict, filename)
