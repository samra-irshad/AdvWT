...
def tensor2im(input_image, imtype=np.uint8):
    ...
    image_pil = Image.fromarray(image_numpy[0].astype(imtype))
...
@torch.no_grad()
def interpolate_using_reference(nets, args, x_src, y_src, x_ref, y_ref, filename):
    N, C, H, W = x_src.size()
    ...
    for i, alpha in enumerate(np.arange(0., 1., 0.1)):
        s_interp = torch.lerp(s_src, s_ref, alpha)
        x_fake = nets.generator(x_src, s_interp, masks=masks)
...
def debug_image(nets, args, inputs, step):
    ...
    filename = ospj(args.sample_dir, '%06d_interpolations_ref.jpg' % (step))
    interpolate_using_reference(nets, args, x_src, y_src, x_ref, y_ref, filename)
